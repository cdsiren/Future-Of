...
const About = (width: number) => {
  ...
  return <>
    <div className={`font-light p-8 w-full min-h-[83vh]`}>
      ...
      <p className="md:w-1/2">Currently building <a target="_blank" href="https://decent.xyz" className={`${width <= 800 ? "hover:text-black hover:bg-white" : "hover:text-white hover:bg-black"} underline`}>decent.xyz</a>, premier NFT infra for Ethereum L2's / high-throughput chains (& Eth L1 but that's a less interesting set of problems & opportunities).  I started researching crypto in 2017, publishing independent work on how Bitcoin was being used by Chinese citizens to <a target="_blank" href="https://github.com/cdsiren/capital-flight" className={`${width <= 800 ? "hover:text-black hover:bg-white" : "hover:text-white hover:bg-black"} underline`}>circumvent capital controls</a>.  My weekend hobby became my weekday career in 2021 when NFTs really pulled me into the space as a builder.</p>
      <p className="md:w-1/2 py-4">I'm particularly interested in discovering where NFTs create evergreen value.  I generally believe property rights are the right framework for evaluating NFTs and that the value they protect is an individual's contributions to an ecosystem.  This is easiest to understand when you picture NFTs without metadata.</p>
      <p>Check out my <a target="_blank" href="https://twitter.com/cdurbinxyz" className={`${width <= 800 ? "hover:text-black hover:bg-white" : "hover:text-white hover:bg-black"} underline`}>Twitter</a> for quick thoughts, <a target="_blank" href="https://www.linkedin.com/in/charlie-durbin-b88544131/" className={`${width <= 800 ? "hover:text-black hover:bg-white" : "hover:text-white hover:bg-black"} underline`}>LinkedIn</a> for work & eduction history, and <a target="_blank" href="https://github.com/cdsiren" className={`${width <= 800 ? "hover:text-black hover:bg-white" : "hover:text-white hover:bg-black"} underline`}>Github</a> for experiments.</p>
    </div>
  </>
}
